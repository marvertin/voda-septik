...
static void pocitani_pulsu(void *pvParameters)
{
    ...
    while(1) {
        ...
        // Výpočet průtoku z časů v bufferu
        ...
        const uint32_t last_idx = (write_idx + FLOW_PULSE_TIMESTAMPS_BUFFER_SIZE - 1U)
                                % FLOW_PULSE_TIMESTAMPS_BUFFER_SIZE;
        last_pulse_time = s_pulse_timestamps[last_idx];
        portEXIT_CRITICAL(&s_pulse_buffer_mux);

        // Kontrola timeoutu - poslední impuls je příliš starý?
        const int64_t time_since_last_pulse = now_us - last_pulse_time;
        if (time_since_last_pulse > FLOW_ZERO_TIMEOUT_US) {
            // Žádný impuls příliš dlouho, průtok je 0
            surovy_prutok = 0.0f;
        } else {
            const int64_t time_span_us = last_pulse_time - first_pulse_time;
            if (time_span_us > 0) {
                // Počet intervalů = FLOW_PULSE_TIMESTAMPS_BUFFER_SIZE - 1
                // (mezi N pulsy je N-1 intervalů, a máme poslední size pulsů)
                const uint32_t intervals = FLOW_PULSE_TIMESTAMPS_BUFFER_SIZE - 1U;
                surovy_prutok = (static_cast<float>(intervals) * 60000000.0f)
                              / (static_cast<float>(time_span_us) * static_cast<float>(s_flow_pulses_per_liter));
            }
        }

        // Zpracovat počet pulsů pro flow counter
        if (sampled_pulses > 0) {
